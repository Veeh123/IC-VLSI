module MEMWB(
	...
	//MODULE OUTPUTS
		...
		output [31:0] 	ALUResult_OUT,		//stored alu result
		output [4:0]  	WriteRegister_OUT,
		output 		WriteEnable_OUT,
		output 		isMemRead_OUT
...
reg 		WriteEnable;
reg [31:0]      ALUResultReg;	//register for storing the ALU result
reg 		isMemReadReg;	//memory read status for forwarding unit
...
assign ALUResult_OUT		= ALUResultReg;
assign isMemRead_OUT		= isMemReadReg;
...
always @(posedge CLOCK or negedge RESET) begin
	...
	//ELSE IF CLOCK IS HIGH
		...
		if(!STALL && !FLUSH) begin
			...
			ALUResultReg    <= ALUResult_IN;
			isMemReadReg	<= isMemRead;
